...
describe("Depósito fallido por saldo insuficiente en cuenta bancaria", () => {
    ...
    it("registra cuenta bancaria y reduce manualmente su saldo a 10", async () => {
        ...
        await models.BankAccount.update({ balance: 10 }, { where: { id: bankAccountId } });
    });
    it("intenta depositar 100 con saldo insuficiente y debe fallar", async () => {
        ...
        expect(res.body.message.toLowerCase()).toMatch(/saldo insuficiente en la cuenta bancaria/);
    });
});
//# sourceMappingURL=depositInsufficientBankBalance.e2e.test.js.map