...
export default function TransferPage() {
  ...
  return (
    <div className="mx-auto max-w-screen-sm p-4">
      <h1 className="text-2xl font-bold mb-4">Transferir dinero</h1>
      <form onSubmit={onSubmit} className="space-y-3 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <label className="text-sm text-gray-600">Alias destino</label>
          <input value={targetAlias} onChange={(e) => setTargetAlias(e.target.value)} className="w-full border rounded-lg p-2" required />
        </div>
        <div>
